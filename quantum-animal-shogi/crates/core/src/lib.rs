use std::{fmt, iter::{once, repeat}, ops::{BitAndAssign, BitOr}, sync::LazyLock};
...
const MAX_TURN: u16 = 256;

// 立っているビットの位置のイテレーターを取得します。

pub fn bits<T: PrimInt + BitAndAssign>(x: T) -> impl Iterator<Item = usize> + Clone {
    let mut x = x;
...
impl fmt::Display for State {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let get_piece_string = |index: usize| {
        ...
        write!(
            f,
            "{}",
            once(format!("手数: {}", self.turn))
                .chain(
                    bits(!self.ownership)
                        .filter(|index| self.bit_boards[*index] == 0)
                        .map(get_piece_string)
                )
                        ...
                        .map(|r| {
                            (0..3)
                                ...
                                .map(|c| {
                                    ...
                                    format!("{}{}", if self.ownership & 1 << index != 0 { "▲" } else { "▼" }, get_piece_string(index))
                                })
                                .rev()
                                .join("　")
                        })
...
impl Game {
    ...
    pub fn lost(state: &State) -> bool {
        bits(!state.ownership).any(|index| state.bit_boards[index] == 0 && state.pieces[index] & 0b_0000_1000 != 0)
    }

    // 手数が256手になって引き分けになったかを取得します。「千日手」はStateが大きくなってしまうので実装していません。
